...
class UserControllerIntegrationTest {
  ...
  void checkPasswordTest() throws Exception {
    // given
    LoginResponse loginResponse = authService.login(new LoginRequest(initDB.getUser1Email(), initDB.getPassword()));
    PasswordCheckRequest request = new PasswordCheckRequest(initDB.getPassword());
    
    // when, then
    mockMvc.perform(post("/api/user/checkpassword")
  ...
  void updateUserTest() throws Exception {
    // given
    LoginResponse loginResponse = authService.login(new LoginRequest(initDB.getUser1Email(), initDB.getPassword()));
    UserUpdateRequest request = new UserUpdateRequest("updated", "updated1!");
